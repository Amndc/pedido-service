...
namespace Pedido.Domain.Custumer.ValueObjects
{
    public record Name
    {
        ...
        public static Name Create(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
                throw new ClienteDomainException("O nome não pode ser vazio");

            name = name.Trim();

            if (name.Length < MIN_LENGTH)
                throw new ClienteDomainException($"O nome deve ter no mínimo {MIN_LENGTH} caracteres");

            if (name.Length > MAX_LENGTH)
                throw new ClienteDomainException($"O nome não pode ter mais que {MAX_LENGTH} caracteres");

            if (!NAME_REGEX.IsMatch(name))
                throw new ClienteDomainException("O nome possui caracteres inválidos");
